...
module Real_Estate_Optimizer
  module PropertylinePick
    def self.pick
      ...
      if selection.length == 1 && selection.first.is_a?(Sketchup::ComponentInstance)
        ...
        if component_contains_closed_loop?(component)
          ...
          handle_property_line_component(component, area)
        ...
        if edges_form_closed_loop?(edges)
          ...
          define_property_line_component(edges, area)
          UI.messagebox("Area: #{convert_to_square_meters(area).round(2)} square meters")
        else
    ...
    def self.handle_property_line_component(component, area)
      model = Sketchup.active_model
      definition_name = "property_line"
      ...
      if existing_definition
        ...
        # Rename component definition to 'property_line' if it does not exist
    ...
    def self.define_property_line_component(edges, area)
      model = Sketchup.active_model
      definition_name = "property_line"
